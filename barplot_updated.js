...
cars.then(function(data) {
    ...
    // Added: tooltip div that stays hidden until the user hovers on a bar
    ...
    barGroups.append("rect")
      ...
      // Added: show tooltip with the exact values on hover
      .on("mouseover", function(event, d) {
          tooltip
            .style("opacity", 1)
            .html(
              `Body style: ${d["body-style"]}<br>` +
              `Drive wheels: ${d["drive-wheels"]}<br>` +
              `Price: ${d.price}`
            );
      })
      // Added: move tooltip with the mouse so it stays readable
      .on("mousemove", function(event) {
          tooltip
            .style("left", (event.pageX + 10) + "px")
            .style("top", (event.pageY - 28) + "px");
      })
      // Added: hide tooltip when the mouse leaves the bar
